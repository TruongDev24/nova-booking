...
export class PaymentController {
  ...
  async handleWebhook(@Req() req: express.Request, @Res() res: express.Response) {
    ...
    try {
      ...
      console.error('PayOS Webhook Verification Note/Error (Expected during test ping):', error.message || error);
      // CRITICAL: Always return 200 OK so PayOS dashboard accepts the URL setup
      return res.status(200).json({ success: true, message: 'Webhook received (registration mode)' });
    }
  }
}
